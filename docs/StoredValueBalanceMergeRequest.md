...
**sourcePaymentMethod** | **kotlin.collections.Map&lt;kotlin.String, kotlin.String&gt;** | The collection that contains the source payment method and its specific information if available. Note that type should not be included since it is inferred from the (target) payment method | 
